...
- **Resolved, not left open (research point 1 above):** build real
  per-ticker static pages (`/stock/NVDA.html`) at build time instead of
  the current `?t=NVDA` query-param routing. This was originally flagged
  as a "still open" decision; closed now because query-param routing
  actively works against a goal this same doc already commits to (Phase
  6's Core Web Vitals bar) — a `?t=TICKER` route means the browser loads
  and runs the full JS bundle before it even knows which ticker to
  render, a real LCP/INP cost a static per-ticker page doesn't pay. The
  pipeline already loops over every ticker, so generating one file per
  ticker is a small addition, not new infrastructure, and it makes a
  ticker page bookmarkable/shareable/linkable — which matters directly
  for a product whose pitch is "here's why this stock's pillars
  disagree." Query-param routing (`?t=TICKER`) stays as a redirect/alias
  so nothing currently linking to it breaks.
